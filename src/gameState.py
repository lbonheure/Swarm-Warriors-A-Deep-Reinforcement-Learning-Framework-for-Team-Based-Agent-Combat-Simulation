...
class GameState:
    ...
    def get_infos(self, agents: dict):
        ...
        for a in agents.keys():
            ...
            resources_pos = []
            walls_pos = []
            self._observe_surrounding(position=(x, y), v_range=3, agents_pos=agents_pos, resources_pos=resources_pos, walls_pos=walls_pos)
    ...
    def _movement(self, agent, direction):
        ...
        match direction:
            ...
            case "S":
                y += 1
        if x<0 or x>=self.grid_width or y<0 or y>=self.grid_height or self.abs_grid[y][x] == "W" or self.abs_grid[y][x] == "R":
            return False
